...
const CAT_NAMES: Record<string, string> = { ai: 'AI', tech: '科技', news: '新闻', finance: '财经', entertainment: '娱乐', general: '综合' };
...
export default function NewsCard({ item, token, onAuthRequired, onSelect }: Props) {
  ...
  return (
    <div id={`news-${item.id}`} className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm hover:border-indigo-500 hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200 cursor-pointer relative" onClick={() => onSelect(item)}>
      <span className={`absolute top-3.5 right-3.5 px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wide uppercase ${CAT_COLORS[cat] || 'bg-gray-100 text-gray-500'}`}>{CAT_NAMES[cat] || cat}</span>
      <h3 className="text-base font-semibold mb-2.5 leading-snug pr-[70px] line-clamp-2 text-gray-900">
        {item.title}
        {item.ai_take && <span className="inline-block ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-600 align-middle">✎</span>}
      </h3>
      ...
      <div className="flex items-center justify-between text-gray-400 text-[12px]">
        ...
        <span className="flex gap-2">
          <button className={`flex items-center gap-1 text-[12px] px-1.5 py-1 rounded-md hover:bg-gray-100 hover:text-gray-600 transition ${favorited ? 'text-amber-500' : ''}`} onClick={toggleFav}>★ {favCount}</button>
          <button className="flex items-center gap-1 text-[12px] px-1.5 py-1 rounded-md hover:bg-gray-100 hover:text-gray-600 transition" onClick={e => { e.stopPropagation(); setShowComments(!showComments); }}>⌨ {item.comments_count || 0}</button>
        </span>
      </div>
      <div className="flex gap-3 mt-2 text-[11px] text-gray-400">
        {pubDate && <span>📰 {pubDate}</span>}
        <span className="opacity-70">⟳ {fetchDate || '-'}</span>
      </div>
